...
> **VGG16 모델과 희소 인덱싱(Sparse Feature Indexing) 기반 유사 이미지 추천 시스템**
> 
...
방법 2 - 채널 활성도 기반 희소 인덱싱
- VGG16 모델은 입력 이미지를 512개의 필터(feature map)에 통과시켜 특징점을 추출한다.
- 유사한 이미지는 높은 반응 강도를 보이는 필터(channel) 또한 유사한 경향을 가진다.
- 따라서 반응 강도가 높은 상위 필터만을 선택하여 희소 인덱스를 생성하고, 이를 기반으로 검색 후보군을 빠르게 필터링한다. <br>생성된 희소 인덱스는 비트 벡터 형태로 저장되며, 비트 연산을 통해 빠른 유사도 비교가 가능하다.
